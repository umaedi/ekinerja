    ...
    <tbody>
        @forelse ($table as $key => $tb)
            <tr>
                ...
                <td>{{ $tb->user->nama }}</td>
                <td>{{ \Carbon\Carbon::parse($tb->created_at)->isoFormat('D MMMM YYYY') }}</td>
                <td><button type="button" onclick="showTugas('{{ $tb->id }}')" class="btn btn-sm btn-success" data-toggle="modal" data-target="#exampleModalCenter"><i class="fa fa-eye"></i></button></td>
            </tr>
        @empty
            <tr>
